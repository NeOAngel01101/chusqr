...
class ChusqersController extends Controller
{
    ...
    public function findchusqerid($id)
    {
        return Chusqer::where('id', $id)->firstOrFail();
    }


    public function like(Request $request)
    {
        $chusqer = $this->findchusqerid();
        $me = $request->user();

        $user->liked()->detach($chusqer);

        return redirect("/")->withSuccess('le has dado like');
    }

    public function dislike(Chusqer $chusqer)
    {
        $chusqer = $this->findchusqerid($id);
        $me = $request->user();

        $user->liked()->detach($chusqer);

        return redirect("/")->withSuccess('le has quitado el like');
    }
